...
class HyperspectralCNN1D(nn.Module):
    """
    1D CNN for hyperspectral pixel classification.
...
def create_model(num_bands, num_classes=11, model_type='resnet', dropout_rate=0.3):
    """
    ...
    Args:
        ...
        model_type: Model architecture type
            - 'cnn': Basic 1D CNN
            - 'resnet': 1D ResNet with residual connections (default)
            - 'deep': Deeper CNN with 5 conv layers
            - 'inception': Inception-style multi-scale CNN
    ...
    if model_type == 'cnn':
        ...
        raise ValueError(f"Unknown model type: {model_type}. Choose from: cnn, resnet, deep, inception, lstm, transformer")
